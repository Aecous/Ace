...
var i18nMap = map[string]map[string]string{
	...
	"flag_enable_fingerprint": {
		LangZH: "启用gogo风格的服务指纹识别(默认开启)",
		LangEN: "Enable gogo-style service fingerprinting (default enabled)",
		LangJA: "gogo風格のサービスフィンガープリント識別を有効化(デフォルト有効)",
	...
	// 速率控制参数
	"flag_ping_rate": {
		LangZH: "设置ICMP扫描速率倍数 (0.0-1.0)",
		LangEN: "Set ICMP scanning rate multiplier (0.0-1.0)",
		LangJA: "ICMPスキャン速度倍数を設定 (0.0-1.0)",
		LangRU: "Установить множитель скорости ICMP-сканирования (0.0-1.0)",
	},
	"flag_ping_timeout": {
		LangZH: "设置ping超时时间(秒)",
		LangEN: "Set ping timeout in seconds",
		LangJA: "pingタイムアウト時間を設定(秒)",
		LangRU: "Установить тайм-аут ping в секундах",
	},
	"flag_local_interface": {
		LangZH: "指定本地网卡IP地址",
		LangEN: "Specify local network interface IP address",
		LangJA: "ローカルネットワークインターフェースのIPアドレスを指定",
		LangRU: "Указать IP-адрес локального сетевого интерфейса",
	},
	"flag_rate_scan_mode": {
		LangZH: "设置速率扫描模式: fast, balanced, stealth",
		LangEN: "Set rate scan mode: fast, balanced, stealth",
		LangJA: "レートスキャンモードを設定: fast, balanced, stealth",
		LangRU: "Установить режим сканирования скорости: fast, balanced, stealth",
	},
	"flag_enhanced_port_scan": {
		LangZH: "启用增强端口扫描 (包含Banner获取、协议识别、未授权检测)",
		LangEN: "Enable enhanced port scanning (includes Banner acquisition, protocol identification, unauthorized detection)",
		LangJA: "拡張ポートスキャンを有効化 (バナー取得、プロトコル識別、未認証検出を含む)",
		LangRU: "Включить расширенное сканирование портов (включая получение баннеров, идентификацию протоколов, обнаружение неавторизованного доступа)",
	},
	...
	"rate_limit_initialized": {
		LangZH: "速率限制初始化完成: %.2f包/秒, 桶容量: %d, 间隔: %v",
		LangEN: "Rate limit initialized: %.2f pps, bucket capacity: %d, interval: %v",
		LangJA: "レート制限初期化完了: %.2f pps, バケット容量: %d, 間隔: %v",
		LangRU: "Инициализация ограничения скорости завершена: %.2f pps, емкость корзины: %d, интервал: %v",
	},
	"traffic_pattern_set": {
		LangZH: "流量模式设置为: %s (%s)",
		LangEN: "Traffic pattern set to: %s (%s)",
		LangJA: "トラフィックパターンを設定: %s (%s)",
		LangRU: "Шаблон трафика установлен: %s (%s)",
	},
	"switching_to_ping": {
		LangZH: "切换为PING方式探测...",
