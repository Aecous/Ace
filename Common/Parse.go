...
func Parse(Info *HostInfo) error {
	...
	// 验证并设置简化的速率扫描模式
	if err := SetRateScanMode(RateScanMode); err != nil {
		LogError(fmt.Sprintf("速率扫描模式设置失败: %v", err))
		RateScanMode = "balanced"
		SetRateScanMode(RateScanMode)
	}

	// 简化的性能监控已集成在RateLimit中，无需额外初始化
