...
func CloseOutput() error {
	...
	if ResultOutput.csvWriter != nil {
		...
		if err := ResultOutput.csvWriter.Error(); err != nil {
			LogDebug(GetText("output_csv_flush_failed", err))
		}
	}

	if ResultOutput.file != nil {
		LogDebug(GetText("output_closing_file"))
		if err := ResultOutput.file.Close(); err != nil {
			LogDebug(GetText("output_close_file_failed", err))
			return fmt.Errorf(GetText("output_close_file_failed", err))
		}
	}

	ResultOutput.isInitialized = false
	LogDebug(GetText("output_close_success"))
	return nil
}
